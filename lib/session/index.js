...
function handleLogin (req, res) {
  if (!req.body.email || !req.body.password) {
    handleLoginFailure(LOGIN_ERROR.PARAMETERS_MISSING);
  } else {
...
function handleLoginFailure (req, res, loginError, err=null) {
  let release = Utils.getRelease(req);
  let isReferred = req.query.referrer === 'explore' || req.query.referrer === 'studyPage';

  let redirectUrl = (function(query) {
    switch(query.referrer) {
      case 'explore':
        return '/explore?restoreState=true&loginError=true';
      case 'studyPage':
        return `/explore/${query.uuid}?loginError=true`;
      default:
        return '/login';
    }
  })(req.query);
  ...
  switch (loginError) {
    case LOGIN_ERROR.ACCOUNT_NOT_FOUND:
      ...
      res.status(403).redirect(redirectUrl);
      break;
    case LOGIN_ERROR.LOGIN_ERROR:
      ...
      if(isReferred) {
        ...
        res.status(500).render('pages/error/500', {release: release});
      }
      break;
